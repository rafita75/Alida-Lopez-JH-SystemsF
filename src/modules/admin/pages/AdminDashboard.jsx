  ...
  const loadDashboardData = async () => {
    try {
      ...
      if (isAdmin || permissions?.canViewInventory) {
        const lowStockRes = await api.get('/inventory/low-stock').catch(() => ({ data: [] }));
        const lowStockVariantsRes = await api.get('/inventory/low-stock-variants').catch(() => ({ data: [] }));
        const totalLowStock = (lowStockRes.data?.length || 0) + (lowStockVariantsRes.data?.length || 0);
        
        setDashboardData(prev => ({
          ...prev,
          productosBajoStock: totalLowStock,
        }));
      }
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white shadow-lg border-t border-gray-100 z-50">
        <div className="flex items-center overflow-x-auto py-2 px-2 gap-1 scrollbar-hide">
          ...
          <button
            ...
            className="flex flex-col items-center gap-1 px-3 py-2 rounded-xl text-red-500 whitespace-nowrap"
          >
                ...
                <div className="bg-white rounded-2xl p-5 shadow-sm hover:shadow-md transition-all border border-gray-100">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      {dashboardData.productosBajoStock > 0 && (
                        <p className="text-xs text-red-500 mt-1">⚠️ Requiere atención</p>
                      )}
                    </div>
